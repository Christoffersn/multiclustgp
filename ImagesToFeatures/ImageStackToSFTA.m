function [meanTextureVectors, STFAFeatureNames] = ImageStackToSFTA(imagePath, startIndex, endIndex, numberlength, fileFormat)
...
features = zeros(frameCount,rows,columns,featureVecSize);
...
parfor i = startIndex+1 : endIndex+1
...
meanTextureVectors = zeros(rows, columns, featureVecSize);

for k = 1:rows
    for l = 1:columns
        for i = 1:featureVecSize
            meanTextureVectors(k,l,i) = mean(features(:,k,l,i));
        end
    end
end
